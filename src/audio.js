// audio.js — Reef Rush synthesized audio bus. ADDITIVE feedback only.
//
...
// external audio files and NO network requests, fully original, tiny. The mp3
// file layer the reference build carried is gone: every event voice below is
// the synth voice itself (playables must run self-contained; sound is a bonus
// the muted-by-default container may never hear).
//
// Architecture (ONE bus, ONE mute, event hooks):
...
export class AudioBus {
  ...
  resume() {
    try {
      ...
      this._started = true;
      if (this._wantAmbient) this._startAmbient(); // bed requested before ready
    } catch (e) {
  ...
  startAmbient() {
    ...
    if (this._ready()) this._startAmbient();
  }
  _startAmbient() {
  ...
  // GOLDEN LAUNCH: the jackpot pig enters the belt — the launch plip carries it.
  onGoldenLaunch() {
    this.onLaunch();
  }

  // GOLDEN PIERCE: the pair's second hit (one shot, two blocks) — a gold-note pop.
  onGoldenPierce() {
    this.onPop('C4');
  }

  // The TREASURE finale: the win arpeggio is the reveal's sting.
  onTreasure() {
    this.onWin();
  }
